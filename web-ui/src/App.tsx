...
export function App() {
  ...
  const [files, setFiles] = useState(false); const [settings, setSettings] = useState(false); const [changes, setChanges] = useState(false); const [workspacePicker, setWorkspacePicker] = useState(false);
  const [meta, setMeta] = useState({ workspace: demo ? "/home/demo/project" : "", workspaceBase: demo ? "/home/demo" : "", workspaceSelected: demo, basePath: "/", codexStatus: demo ? "ready" : "starting", codexVersion: "Codex" });
  ...
  const onMessage = useCallback((message: any) => {
    if (message.type === "status") setMeta((old) => ({ ...old, ...message }));
      ...
      setActive(message.thread); setItems(queued ? [{ type: "user_message", id: queued.id, text: queued.text, attachments: queued.attachments.map((item) => ({ name: item.name, kind: item.kind, data: item.kind === "image" ? item.data : undefined })) }] : nextItems); setDiff(diffFromItems(nextItems)); setThreadUsage(null); setCompaction(message.compaction || null); setRunning(Boolean(queued));
      ...
      if (activeRef.current?.id === message.threadId && message.action !== "unarchive") { setActive(null); setItems([]); setDiff(""); setThreadUsage(null); setCompaction(null); }
    ...
    else if (message.type === "fs.entries") { pending.current.get(`list:${message.path}`)?.(message.entries); pending.current.delete(`list:${message.path}`); }
    else if (message.type === "fs.file") { pending.current.get(`read:${message.file.path}`)?.(message.file); pending.current.delete(`read:${message.file.path}`); }
    else if (message.type === "fs.saved") { pending.current.get(`write:${message.file.path}`)?.(message.file); pending.current.delete(`write:${message.file.path}`); }
    ...
    else if (message.type === "workspace.selected") { setMeta((old) => ({ ...old, ...message, workspaceSelected: true })); setWorkspacePicker(false); setActive(null); setItems([]); setDiff(""); setThreadUsage(null); setCompaction(null); }
  ...
  const create = () => {
    setDiff(""); setItems([]); setActive(null); setThreadUsage(null); setCompaction(null); setRunning(false);
  ...
  const select = (id: string) => demo ? (setActive(demoThreads.find((entry) => entry.id === id) || threads.find((entry) => entry.id === id) || null), setItems(id === "demo" ? demoItems : []), setDiff(id === "demo" ? diffFromItems(demoItems) : "")) : send({ type: "thread.resume", threadId: id });
  ...
  const request = useCallback(<T,>(key: string, message: any) => new Promise<T>((resolve) => {
    ...
    window.setTimeout(() => {
      let value: any;
      if (message.type === "fs.read") value = { path: message.path, content: "import { z } from 'zod';\n\nconst userSchema = z.object({\n  email: z.string().email(),\n  name: z.string().min(1),\n});\n\nexport async function createUser(req, res) {\n  const result = userSchema.safeParse(req.body);\n  if (!result.success) {\n    return res.status(400).json({ error: 'Invalid input' });\n  }\n}" };
      else if (message.type === "fs.write") value = { path: message.path, content: message.content };
  ...
  const loadFiles = useCallback((path: string) => request<FileEntry[]>(`list:${path}`, { type: "fs.list", path }), [request]);
  const readFile = useCallback((path: string) => request<{ path: string; content: string }>(`read:${path}`, { type: "fs.read", path }), [request]);
  const writeFile = useCallback((path: string, content: string) => request<{ path: string; content: string }>(`write:${path}`, { type: "fs.write", path, content }), [request]);
  ...
  return <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    ...
    {files && <FileDrawer close={() => setFiles(false)} load={loadFiles} read={readFile} write={writeFile} onSaved={(path, before, after) => setDiff((current) => [current, editDiff(path, before, after)].filter(Boolean).join("\n"))} />}
